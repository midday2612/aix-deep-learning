...
LSTM 모델이 CNN 모델보다 약 41.045 초 더 소요되었다. 모델의 학습 및 예측 속도에 대한 추가 최적화가 필요할 수 있다.LSTM 모델이 조금 더 높은 정확도를 보이고 있으며, 따라서 현재 상황에서는 더 성능이 좋은 모델로 판단된다.모델의 학습 속도를 향상시키기 위해 하이퍼파라미터를 바꿔가며 테스트 하고 데이터 증강 등의 방법을 통해 모델의 과적합 문제를 해결해나갈 수 있다. 
