...
## 데이터 로드 및 전처리
   ...
   1) ### 데이터 로드
      ...
      [네이버영화리뷰데이터](https://github.com/e9t/nsmc/)
      네이버영화에서 추출한 한국어로 된 영화 리뷰 데이터이다.
      ``` python
      import pickle
      import pandas as pd
      import numpy as np
      ...
      **데이터 소개**
      ...
      총 리뷰는 다음과 같이 이루어져있다.
      - rating.txt : 20만개의 전체 리뷰 (훈련 데이터)
      - rating_test.txt : 테스트를 위해 보류된 50,000개의 리뷰 (테스트데이터)
      - rating_train.txt : 트레이닝 용 리뷰 150,000개
     
      rating.txt(훈련 데이터)와 rating_test.txt(테스트 데이터)를 로드한다. 이때 직접 파일을 가져오는 경우, 데이터 셋이 바뀔 가능성이 있어 로컬 파일에 데이터를 저장한다. 외부데이터에 대한 의존성을 줄여 데이터셋의 안정성을 높인다.
